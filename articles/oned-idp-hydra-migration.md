...
そこで最初の ADR を置き換える形で、**Ory Hydra + 自前 IDM（Identity Management。ユーザー管理サービス）** の構成に方針転換しました。
...
- **データモデル。** Kratos はユーザー属性（traits）を JSON Schema で定義し、DB 上も JSON で保持します。BI ツールで見るだけならビューで平坦化できますが、後述するように既存ユーザーの移行では移行期間中に既存 DB からテーブル単位で同期し続ける方式を想定しており、同期先のスキーマを自分たちで握っている必要がありました。Kratos のインポート API で一括投入すること自体はできますが、並行稼働期間の継続同期は別途作ることになります
- **パスワードポリシー。** Kratos のパスワードポリシーは NIST SP 800-63B 準拠の方針で固定されていて、調整できるのは最小文字数や漏洩パスワードチェックの有無くらいです。既存サービスから引き継いだ自社ルールはこの枠に収まらず、サーバ側で表現できません。ポリシー検証が走るのはパスワードを新しく設定する場面だけで、ログインはハッシュ比較のみなので、対応形式のハッシュをインポートすれば既存パスワードでのログイン自体はできます。ただし既存ユーザーがパスワードを変更・再設定した瞬間にルールが変わる体験になり、既存ユーザーの体験を変えないという今回の方針と合いませんでした
...
代わりに、Kratos なら標準で備わるパスワードポリシー、アカウント復旧、多要素認証などは自前で持つことになります。ここは Hydra に委譲できる範囲（プロトコル）と自前で持つ範囲（データとフロー）の線をどこに引くか、という判断でした。
...
Web の RP は better-auth が自前の DB にセッションとユーザーを持ち、1D API はそれとは別の DB に会員の users テーブルを持ちます。その結果、1 人のユーザーに対して ID が 3 つ存在するようになります。

| ID | 実体 |
| --- | --- |
| better-auth の `user.id` | RP（1D Web）が自前 DB に持つセッション管理用ユーザーの主キー |
| IdP の `sub` | Hydra が発行する主体識別子。1D API 側の users テーブルに紐付けとして保存される |
| 会員 ID | 1D API 側の users テーブルの主キー。課金・計測が使う |
