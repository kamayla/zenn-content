...
- RP 実装では Next.js の `<Link>` prefetch、better-auth の `nextCookies()`、3 種類のユーザー ID の混同に注意
...
- **パスワードポリシー。** Kratos のパスワードポリシーは、調整できる範囲が最小文字数や漏洩パスワードチェックの有無などに限られ、独自ルールを差し込む拡張点がありません。既存サービスから引き継いだ自社ルールはこの枠に収まらず、サーバ側で表現できません。ポリシー検証が走るのはパスワードを新しく設定する場面だけで、ログインはハッシュ比較のみなので、対応形式のハッシュをインポートすれば既存パスワードでのログイン自体はできます。ただし既存ユーザーがパスワードを変更・再設定した瞬間にルールが変わる体験になり、既存ユーザーの体験を変えないという今回の方針と合いませんでした。自前で持てばポリシーは IdP 側に閉じるので、今後見直す際も各プロダクトに手を入れずに変更できます
...
Hydra に登録する OAuth2 クライアントは JSON ファイルを source of truth にして、起動時に宣言的に同期しています。`kubectl apply` のように「ファイルにある状態に Hydra を合わせる」動きで、クライアントの追加は JSON を 1 ファイル置くだけです。本番では、ファイルに無いクライアントを自動では削除せず、差分を警告として出して人が判断する運用にしています。誤マージで本番クライアントが消えると、その RP の全ユーザーがログイン不能になるためです。

## Hydra の設定で意識的に決めたこと

- **PKCE を全クライアントに強制。** confidential なサーバサイド RP も含めて例外なし
- **`sub` は public 方式。** RP ごとに異なる `sub` を返す pairwise 方式はプライバシー面では優れますが、Hydra では JWT 形式のアクセストークンと併用できません。今回はファーストパーティのプロダクト群なので、全 RP で同じ `sub` を返す public を選んでいます
- **Admin API は外部に公開しない。** ログインの accept など強い操作ができる Admin API は、idm-backend からの内部通信だけが到達できる構成にしています
...
| IdP の `sub`             | idm-backend がログイン accept 時に渡す主体識別子（IdP 側 users の主キー）。Hydra がトークンの `sub` に載せ、1D API 側の users テーブルに紐付けとして保存される |
| 会員 ID                  | 1D API 側の users テーブルの主キー。課金・計測が使う                                                                                                           |

これらは互いに別の名前空間で、better-auth の `user.id` を会員 ID として計測に流すと別人相当の値を送ることになります。社内の RP 実装ガイドでは「最も間違えやすい箇所」として冒頭に置き、会員 ID は必ず GraphQL の `me` から取る、というルールにしています。
